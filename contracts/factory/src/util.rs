...
pub fn verify_creation_fee_paid(factory: &crate::factory::FactoryData) -> Result<(), &'static str> {
    if factory.creation_fee_ckb == 0 {
        return Err("Creation fee not set");
    }
    Ok(())
}

pub fn verify_launch_fee_paid(required_fee: u64) -> Result<(), &'static str> {
    if required_fee == 0 {
        return Err("Launch fee not set");
    }
    Ok(())
}

pub fn verify_pool_created(_dex_data: &crate::factory::DexInstanceData) -> Result<(), &'static str> {
    Ok(())
}
